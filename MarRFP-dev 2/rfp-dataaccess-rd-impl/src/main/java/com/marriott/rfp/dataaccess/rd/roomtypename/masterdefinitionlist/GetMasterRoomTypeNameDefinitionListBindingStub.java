...
public class GetMasterRoomTypeNameDefinitionListBindingStub extends org.apache.axis.client.Stub implements GetMasterRoomTypeNameDefinitionListPortType {
	private Vector cachedSerClasses = new Vector();
	private Vector cachedSerQNames = new Vector();
	private Vector cachedSerFactories = new Vector();
	private Vector cachedDeserFactories = new Vector();

	public GetMasterRoomTypeNameDefinitionListBindingStub() throws org.apache.axis.AxisFault {
		this(null);
	}

	public GetMasterRoomTypeNameDefinitionListBindingStub(java.net.URL endpointURL, javax.xml.rpc.Service service) throws AxisFault {
		this(service);
		super.cachedEndpoint = endpointURL;
	}

	public GetMasterRoomTypeNameDefinitionListBindingStub(javax.xml.rpc.Service service) throws AxisFault {
		if (service == null) {
		...
		qName = new QName("", ">MI_MasterRoomTypeNameDefinitionListRS");
		cachedSerQNames.add(qName);
		cls = MI_MasterRoomTypeNameDefinitionListRS.class;
		cachedSerClasses.add(cls);
		cachedSerFactories.add(beansf);
		cachedDeserFactories.add(beandf);

		qName = new QName("http://www.opentravel.org/OTA/2003/05", ">MI_MasterRoomTypeNameDefinitionListRS");
		cachedSerQNames.add(qName);
		cls = MI_MasterRoomTypeNameDefinitionListRS.class;
		cachedSerClasses.add(cls);
		cachedSerFactories.add(beansf);
		...
		qName = new QName("http://www.opentravel.org/OTA/2003/05", ">MI_MasterRoomTypeNameDefinitionListRQ");
		cachedSerQNames.add(qName);
		cls = MI_MasterRoomTypeNameDefinitionListRQ.class;
		cachedSerClasses.add(cls);
		cachedSerFactories.add(beansf);
		cachedDeserFactories.add(beandf);
	...
	public MI_MasterRoomTypeNameDefinitionListRS MI_MasterRoomTypeNameDefinitionListRQ(MI_MasterRoomTypeNameDefinitionListRQ MI_MasterRoomTypeNameDefinitionListRQ,String SoapPort_address) throws RemoteException, ServiceException {
		if (super.cachedEndpoint == null) {
			throw new org.apache.axis.NoEndPointException();
		}
		Call _call = createCall();
		_call.addParameter(
			new QName("http://www.opentravel.org/OTA/2003/05", "MI_MasterRoomTypeNameDefinitionListRQ"),
			new QName("http://www.opentravel.org/OTA/2003/05", ">MI_MasterRoomTypeNameDefinitionListRQ"),
			MI_MasterRoomTypeNameDefinitionListRQ.class,
			ParameterMode.IN);
		_call.setReturnType(new QName("http://www.opentravel.org/OTA/2003/05", ">MI_MasterRoomTypeNameDefinitionListRS"), MI_MasterRoomTypeNameDefinitionListRS.class);
		_call.setUseSOAPAction(true);
		_call.setSOAPActionURI("GetMasterRoomTypeNameDefinitionList");
		_call.setEncodingStyle(UriLiteralConstant.URI_LITERAL_ENC);
		_call.setOperationStyle("document");
		_call.setOperationName(new QName("http://www.opentravel.org/OTA/2003/05", "MI_MasterRoomTypeNameDefinitionListRQ"));

		java.lang.Object _resp;
		try {
			_resp = _call.invoke(new Object[] { MI_MasterRoomTypeNameDefinitionListRQ });
			if (_resp instanceof java.rmi.RemoteException) {
				throw (java.rmi.RemoteException) _resp;
			} else {
				try {
					return (MI_MasterRoomTypeNameDefinitionListRS) _resp;
				} catch (Exception _exception) {
					return (MI_MasterRoomTypeNameDefinitionListRS) JavaUtils.convert(_resp, MI_MasterRoomTypeNameDefinitionListRS.class);
				}
			}
		} catch (Exception e) {
	...
	public MI_MasterRoomTypeNameDefinitionListRS MI_MasterRoomTypeNameDefinitionListRQ(String SoapPort_address) throws RemoteException, ServiceException {
		return MI_MasterRoomTypeNameDefinitionListRQ(GetRequestMessage(),SoapPort_address);

	}

	private MI_MasterRoomTypeNameDefinitionListRQ GetRequestMessage() {
		MI_MasterRoomTypeNameDefinitionListRQ req_msg = new MI_MasterRoomTypeNameDefinitionListRQ();
