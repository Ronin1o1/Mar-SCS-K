...
const API = {
  async getAccGrpQuestions(accountrecid: number, period: number) {
    
    let res = await axios.get(`${Utils.getAPIURI(Settings.api.getAccGrpQuestions)}`, {
    ...
    API.handleErrorResponse(res, Settings.api.getAccGrpQuestions);
  ...
  async download() {
    axios({
      ...
      link.href = url;
      link.setAttribute('download', `${Settings.Questions.fileName}.xlsx`); //or any other extension
  ...
  async importExcelData(file: any, accountrecid: any) {
    ...
    excelData.append('gmqfile', file, file.name)
