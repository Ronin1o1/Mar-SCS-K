...
export const SelectHotelContextProvider = (props) => {
  ...
  const handleChangeProperty = (e) => {
    ...
    if (value && value.length > 0) {
      ...
      sessionStorage.setItem("MARSHACODE", value[0].marshaCode);
      sessionStorage.setItem("HOTELNAME", value[0].hotelName);
    }
  };
  ...
  const handleHotelChange = (event) => {
    ...
    sessionStorage.setItem("MARSHACODE", event.target.value);
    ...
    if (
      ...
      roomTypeNameDefinitionSample.roomTypeNameDefinition.filter(function (
        ...
        if (item.availabilityInd == "Y") {
          ...
          if (item.unitOfMeasure != null) item.unitOfMeasure.uom_Code = "";
        }
      });
    }
