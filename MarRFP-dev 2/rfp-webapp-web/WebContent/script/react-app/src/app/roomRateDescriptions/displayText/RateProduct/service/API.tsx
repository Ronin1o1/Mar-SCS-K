...
const API = {
  ...
  async deleteChannelLang(data: any, selectedChannel: any) {
    ...
    const strLanguage = { code: splitData2[0], englisName: splitData2[1] };
    const params = {
  ...
  /** Data Elem -- START */
  async getProductDisplayAPI(data: any) {
    const strChannel = {
    ...
    const postData = Utils.createPostData(params);
    const res = await axios.post(
      `${Utils.getAPIURI(Settings.api.getProductDisplay)}`,
      postData,
      {
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
    ...
    if (res && res.status !== 302) {
      API.handleErrorResponse(res, Settings.api.getProductDisplay);
    }
    return res.data;
  },
  async updateProductDisplayAPI(data: any, path: any) {
    const strChannel = {
    ...
    const postData = Utils.createPostData(params);
    const res = await axios.post(
      Utils.getAPIURI(Settings.api.updateProductDisplay),
      postData,
      {
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
    ...
    API.handleErrorResponse(res, Settings.api.updateProductDisplay);
  ...
  async getAmenity(data: any) {
    ...
    if (res && res.status !== 302) {
      API.handleErrorResponse(res, Settings.api.getAmenity);
    }
    return res.data;
  },
  ...
  async updateAmenityAPI(data: any, path: any) {
    ...
    const params = {
      strTheBrand: JSON.stringify(data.strTheBrand),
    ...
    API.handleErrorResponse(res, Settings.api.updateAmenity);
