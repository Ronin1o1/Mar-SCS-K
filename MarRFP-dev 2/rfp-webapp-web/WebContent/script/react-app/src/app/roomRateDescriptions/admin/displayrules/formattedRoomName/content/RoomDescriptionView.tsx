...
function RoomDescriptionView() {
  ...
  const payloadJson = () => {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      for (let j = 0; j < arr[i].elementGroups[0].elements.length; j++) {
        elementCodeList = arr[i].elementGroups[0].elements[j].elementCodeList;
        elementCode = arr[i].elementGroups[0].elements[j].elementCode;
        const data = {
    ...
    localStorage.setItem(
      "formattedRoomNameData",
      JSON.stringify({ params: params })
    );
    return params;
  };
  const saveDetails = (updateData) => {
    const savedData = JSON.parse(localStorage.getItem("formattedRoomNameData"));
    ...
    if (params) {
      ...
      API.upDataElementRules(params)
        .then((res) => {
          ...
          localStorage.removeItem("formattedRoomNameData");
        })
        .catch((error) => {
          setLoader(false);
          localStorage.removeItem("formattedRoomNameData");
  ...
  const handleNext = () => {
    if (allstate.maxLines == null || allstate.maxLines == "") {
      ...
      //saveDetails(false);
      history.push({
        pathname: "/roomtypenamerules/formattedRoom",
      });
    }
  };
        ...
        return (
          <div>
            <div style={{ height: "97vh" }}>
              <CPageTitle
                title={Settings.roomDescriptionTitles.ruleEntry}
              ></CPageTitle>
                                  ...
                                  <table className={styles.zeroHeight}>
                                    <tbody>
                                      <tr>
                                        ...
                                        <td className={styles.fieldName}>
                                          {Settings.roomDescriptionTitles.entry}
                                        </td>
                                        <td className={styles.field}></td>
                                        <td className={styles.field_Value}>
                                          {history.location.state.strEntry.name}
                                        </td>
                                      </tr>
                                    </tbody>
                                  </table>
                                ...
                                <td className={styles.zero}>
                                  <table className={styles.zeroHeight}>
                                    <tbody>
                                      <tr>
                                        <td className={styles.fieldName}>
                                          {Settings.roomDescriptionTitles.mode}
                                        </td>
                                        <td>
                                          <CSelect
                                            name="type"
