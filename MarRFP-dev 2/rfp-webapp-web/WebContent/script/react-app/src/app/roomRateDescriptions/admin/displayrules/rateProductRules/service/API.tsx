...
const API = {
  ...
  async availableElementRules(data: any) {
    // For API call for post method rules elements
    const postData = API.createPostData(data);
    const res = await axios.post(
    ...
    API.handleErrorResponse(res, Settings.api.availableElementRules);
    return res.data;
  },

  async upDataElementRules(data: any) {
    // For API call for post method updata elementRules
    const postData = API.createPostData(data);
    const res = await axios.post(
