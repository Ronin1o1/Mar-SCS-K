...
export default function Language(props) {
  ...
  const onViewBtnClick = (event) => {
    ...
    if (event.keyCode === 13) {
      ...
      if (focusedElem.id && focusedElem.id.includes("dispTextFrmRmName")) {
  ...
  function viewButtonTemplate(rowData) {
    return (
      <div tabIndex={0} id={`dispTextFrmRmName-${rowData.code}`}>
        <img
          src={BtnView}
          className={classnames(styles.deleteContainer, styles.cursorPointer)}
          onClick={() => {
            sessionStorage.removeItem("radioBtnchecked-FRN");
            props.data.getCopyText(
              rowData.code,
              rowData.englishName,
              false,
