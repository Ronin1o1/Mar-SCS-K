...
import API from "../../../app/pricing/portfolio/hotelSolicitation/service/API";
import CCalendarHotel from "../../common/components/CCalendarHotel";

function sendAdditionalEmailHotel(props) {
  ...
  useEffect(() => {
    if (hotelContext) {
      if (hotelContext.hotelSolicitationAddEmailInfo?.additional_text) {
        setTextareaValue(
          hotelContext.hotelSolicitationAddEmailInfo?.additional_text
        );
      } else {
        setTextareaValue("");
      }
      if (hotelContext.hotelSolicitationAddEmailInfo?.sendfromtype) {
        setdropdownValue(
          hotelContext.hotelSolicitationAddEmailInfo?.sendfromtype
        );
      } else {
        setdropdownValue(1);
      }
      if (hotelContext?.hotelSolicitationAddEmailInfo?.shortDuedate_foremail) {
        setdateValue(
          hotelContext?.hotelSolicitationAddEmailInfo?.shortDuedate_foremail
        );
      } else {
        setdateValue("");
      }
  ...
  useEffect(() => {
    document.title = "Additional Email Information";
    const hotelContext = sessionStorage.getItem("sendFromList");
    if (hotelContext) {
      setHotelContext(JSON.parse(hotelContext));
    }
  }, []);
  ...
  const setCalendarValue = (param) => {
    ...
    if (isPastDate && !isToday(param.value)) {
      ...
      if (date == "Invalid date") {
        const emailInfoId = document.getElementById("additionalEmailInfoId");
        emailInfoId.blur();
        setdateValue(param.value);
      } else {
        setdateValue(date);
      }
    }
  ...
  const clear = () => {
    ...
    setdropdownValue(
      hotelContext.hotelSolicitationAddEmailInfo
        ? hotelContext.hotelSolicitationAddEmailInfo.sendfromtype
        : []
    );
  };
  ...
  const options = () => {
    const items = [];
    if (hotelContext?.emailtypelist) {
      for (const element of hotelContext.emailtypelist) {
        const row =
  ...
  const sendEmailHandler = (param, paneldata) => {
  ...
  const saveHandlerEmail = () => {
    ...
    const screentype = hotelContext.hotelSolicitationAddEmailInfo
      ? hotelContext.hotelSolicitationAddEmailInfo.addemailtext_screentype
      : "S";
    ...
    const data = {
      ...
      sendfromemail: hotelContext.hotelSolicitationAddEmailInfo
        ? hotelContext.hotelSolicitationAddEmailInfo.sendfromemail
        : null,
    ...
    const panelDataStr = sessionStorage.getItem("panelData");
    const panelData = panelDataStr ? JSON.parse(panelDataStr) : {};
    //alert(panelData);
    sendEmailHandler(data, panelData);
  };

  const handleBlurDate = (e) => {
    let dt = e === "" ? "" : e.target.value;
    let validDate = true;
    validDate = dt !== "" && dt !== null ? Utils.isDate(dt) : false;
    const isPastDate = isPast(dt);
    if (dt !== "" && dt !== null) {
      if (isPastDate && !isToday(dt)) {
        alert(Settings.pastDate);
        setdateValue("");
      } else {
        if (moment(dt).toDate().getFullYear() > 9999) {
          setdateValue("");
        } else if (validDate) {
              ...
              <table>
                <tr>
                  ...
                  <td className={styles.field_Value}>
                    <textarea
                      id={Settings.additionalSendMail.textareaId}
                      name={Settings.additionalSendMail.textareaName}
                      cols={Settings.additionalSendMail.textareaCols}
                ...
                {hotelContext?.hotelSolicitationAddEmailInfo != null ? (
                  hotelContext?.hotelSolicitationAddEmailInfo
                    ...
                    <tr>
                      <td className={styles.duedate_value}>
                        <input
                          type="hidden"
                          name={Settings.additionalSendMail.dateInputName}
                          id={Settings.additionalSendMail.dateInputId}
                          maxLength={10}
                        ...
                        <CCalendarHotel
                          ...
                          value={
                            dateValue == "" || dateValue == null
                              ? ""
                              : Utils.convertStrToDate(dateValue)
                          }
                ...
                <tr>
                  ...
                  <td className={styles.duedate_value}>
                    <select
                      name={Settings.additionalSendMail.sendfromtypeName}
                      id={Settings.additionalSendMail.sendfromtypeId}
                      value={dropdownValue}
                      onChange={(e) => {
                        onChangeDropdown(e);
                      }}
              ...
              <table className="menuHght100-Height">
                <tr>
                  ...
                  <td className={styles.save}>
                    <img src={btnSave} onClick={saveHandlerEmail} />
                  </td>
...
export default sendAdditionalEmailHotel;
