...
const API = {
  async getLimitedSalesUsersData(postData, pageNumber) {
    const formattedPostData = createPostData(postData, pageNumber);
    const res = await axios.post(
      Utils.getAPIURI(Settings.api.getLimitedSalesUsersUrl),
      formattedPostData,
      {
  ...
  async postLimitedSalesUsersData(data) {
    const params = {
    ...
    const res = await axios.post(
      `${Utils.getAPIURI(Settings.api.updateLimitedSalesUsersUrl)}`,
      postData,
      {
        headers: {
