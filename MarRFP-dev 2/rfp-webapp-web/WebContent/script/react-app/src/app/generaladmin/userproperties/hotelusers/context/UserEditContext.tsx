...
export const UserEditContextProvider = (props) => {
  ...
  const onClickSearch = (pageNumber = 1, type) => {
    ...
    if (type == Settings.labels.availPropText) {
      ...
      if (searchData?.user?.enhancedReportingChk == "Y") {
        regionParams["enhancedReporting"] = "on";
      }
      ...
      if (searchData?.user?.enhancedReportingChk == "Y") {
        brandParams["enhancedReporting"] = "on";
      }
      ...
      if (searchData?.user?.enhancedReportingChk == "Y") {
        franchiseParams["enhancedReporting"] = "on";
      }
    ...
    if (data) {
      ...
      if (data.userDetails) {
        ...
        hotelData.user.enhancedReportingChk =
          data.userDetails.enhanced_reporting == "Y" ? "Y" : "N";
  ...
  const hotelsUpdateProp = (data) => {
    ...
    if (strHotelUpdate != "") {
      ...
      strHotelToArr = strHotelUpdate.split(",");
    }
      ...
      if (data && data !== "error") {
        ...
        APIEdit.updateSelectPropList(updateParams).then((data) => {
          if (data.status == "success") {
            ...
            hotelUpdateData.user.numHotelItems = data.totNumUserProp
              ? data.totNumUserProp
              : 0;
      ...
      if (data.status == "success") {
        ...
        APIEdit.updateSelectPropList(deleteParams).then((data) => {
          if (data.status == "success") {
            ...
            hotelDeleteData.user.numHotelItems = data.totNumUserProp
              ? data.totNumUserProp
              : 0;
      ...
      if (!re.test(data)) {
        ...
        APIEdit.populatePropList(qucikSelectParam).then((data) => {
          ...
          if (data.notfound && data.notfound === "") {
            ...
            sessionStorage.setItem("quickSelectMessage", data.notfound);
          }
  ...
  const setPropertyType = (type) => {
    ...
    const popupMsg =
      type == "Property"
        ? Settings.alerts.switchToProperty
        : type == "Region"
        ? Settings.alerts.switchToRegion
        : type == "Brand"
        ? Settings.alerts.switchToBrand
        : type == "Franchise"
        ? Settings.alerts.switchToFranchise
        : Settings.alerts.switchToAllProperties;
    if (!confirm(popupMsg)) {
      return false;
    } else {
      msg = true;
  ...
  const updateallhotelbtn_onclick = (data, calledFrom) => {
    ...
    APIEdit.updateAll(params).then((data) => {
      if (data.status == "selPropList") {
        allData.user.allhotels = data.allHotels;
        allData.user.allhotels = data.allHotels;
        ...
        allData.isRegion = false;
        allData.isAll = false;
        allData.user.numHotelItems = data.totNumUserProp
          ? data.totNumUserProp
          : 0;
        setState(allData);
        setIsLoaded(true);
        updateSelectPropList();
      }
    });
  };
  ...
  const onCopyClickSearch = (pageNumber = 1) => {
    ...
    const scrollContent = document.getElementById("copyGridTable");
    if (scrollContent) {
  ...
  const updateCopy = () => {
    ...
    if (propertyData?.user?.enhancedReportingChk == "Y") {
      updateCopyParams["enhancedReporting"] = "on";
    }
  ...
  const updateSelectPropList = () => {
    ...
    if (hotelUpdateData?.user?.enhancedReportingChk == "Y") {
      params["enhancedReporting"] = "on";
    }
