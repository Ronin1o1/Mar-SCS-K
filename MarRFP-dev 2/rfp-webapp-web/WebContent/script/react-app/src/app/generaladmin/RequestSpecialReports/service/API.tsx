...
const API = {
  async getReportType() {
    const res = await axios.get(Utils.getAPIURI(Settings.api.getReports), {
      headers: {
        Pragma: "no-cache",
      },
    });
  ...
  async getReportFilteredData(filterValue) {
    const params = { strFilterValues: JSON.stringify({ report: filterValue }) };
    const postData = Utils.createPostData(params);

    const res = await axios.post(
      Utils.getAPIURI(Settings.api.getReports),
      postData,
      {
        headers: {
